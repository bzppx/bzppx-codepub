...
CREATE TABLE `cp_module` (
  ...
  `pre_command_exec_timeout` int(11) NOT NULL COMMENT '前置命令超时时间,单位秒',
  `post_command` text NOT NULL COMMENT '后置命令',
  `post_command_exec_type` int(11) NOT NULL COMMENT '后置命令执行方式, 1 同步执行，遇到错误停止;2 同步执行，遇到错误继续;3 异步执行',
  `post_command_exec_timeout` int(11) NOT NULL COMMENT '后置命令超时时间,单位秒',
  `exec_user` varchar(30) NOT NULL COMMENT '执行命令用户',
